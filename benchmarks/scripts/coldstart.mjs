 ...
 * measures:
 *   1. "Full process time"    - spawn -> exit.
 *   2. "Import + first init"  - import to first Database.knex() call.
 *
...
printTable({
    ...
    headers: ["Metric", "Baseline (0.1.21)", "Optimized", "Speedup"],
    rows: [
        {
            cells: [
...
const table = [
    ...
    `| Import → first init | ${baseInt.median.toFixed(1)}ms | ${optInt.median.toFixed(1)}ms | **${(baseInt.median / optInt.median).toFixed(2)}x** |`
].join("\n");
